...
with st.sidebar:
    ...
    page = st.radio(
        "Navigation",
        ["💬 Chat", "📚 Knowledge Base", "⚙️ Settings"],
        label_visibility="collapsed",
    )

    st.divider()
    st.markdown("**System status**")
    st.markdown(f"{'🟢' if chunk_count > 0 else '🔴'} Vector index ({chunk_count} chunks)")
    st.markdown(f"{'🟢' if has_api_key else '🔴'} OpenRouter API key")
        ...
        with st.chat_message(message["role"]):
            ...
            if message["role"] == "assistant":
                ...
                with translation_col:
                    if st.button("🔤 Arabic", key=f"arabic_btn_{idx}"):
    ...
    if user_query:
        ...
        with st.chat_message("assistant"):
            ...
            if result["avg_similarity"] < LOW_CONFIDENCE_THRESHOLD and result["sources"]:
                st.warning(
                    "Low retrieval confidence - verify against primary literature.",
                    icon="⚠️",
                )
            if result["sources"]:
